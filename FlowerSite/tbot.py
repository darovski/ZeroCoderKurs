...
from flower_app.models import Order, Cart, Product, User, CartItem
...
async def process_address(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        logger.info(f"Returning ORDER_DATE state: {ORDER_DATE}")
        return ORDER_DATE
    except Exception as e:
...
def main() -> None:
    ...
    # 1. Сначала определяем ConversationHandler
    ...
    # 2. Теперь добавляем его в application
    application.add_handler(conv_handler)

    # 3. Затем добавляем остальные обработчики
